define('bloompeak-jr-dashboard-items/report', ['underscore', 'jquery', 'wrm/context-path'], function (_, $, contextPath) {
  ...
  function renderInternal(API, context, isEdit) {
    ...
    var $element = this.$element = $(context).find(`#bloompeak-jr-static`);
    ...
    if(frameElem===null){
      ...
      $element.empty().html(Dashboard.Item.Templatesbloompeakjr.IFrame({ frameId:frameId, dashboardId: dashboardId, dashboardItemId: dashboardItemId, isEdit: isEdit, contextPath: contextPath() }));
    }else{
